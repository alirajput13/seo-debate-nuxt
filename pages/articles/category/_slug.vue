...
export default {
  ...
  computed: {
    ...
    filteredTags: function () {
      return this.allArticles.filter(
        (blog) => blog.category === this.data.category
      );
    },
  },
  methods: {
  ...
  mounted() {
    this.data = this.allArticles.find(
      (blog) => this.whiteSpace(blog.category) == this.$route.params.slug
    );
  },
