      ...
      ----------------------------------------------- -->
        ...
        >
            ...
            <div class="bg-light bottom-spacer no-underline">
                <v-container>
                    <v-row>
                        <v-col v-if="$store.state.termsConditionLayout.showFAQ" class="bg-light bottom-spacer no-underline" cols="12">
                            <Faqs :faqs="$store.state.termsConditionLayout.faq" />
                        </v-col>
                    </v-row>
                </v-container>
            </div>
            <div v-if="$store.state.termsConditionLayout.showB">
                <SeoTips
                    :layout="{
                        h: $store.state.termsConditionLayout.header2,
                        s: $store.state.termsConditionLayout.sub2,
                        c: $store.state.termsConditionLayout.content
                    }"
                />
            </div>
            <!-- <SeoTips /> -->
...
export default {
    ...
    head() {
        return {
            ...
            link: [
                {
                    rel: 'canonical',
                    href: this.$store.state.termsConditionLayout.canonical_link || 'https://seodebate.com' + this.$route.path
                }
            ],
            meta: [
                {
                    name: 'robots',
                    content: !!+this.$store.state.termsConditionLayout.index ? 'index,follow' : 'noindex'
                },
                {
                    name: 'title',
                    content: this.$store.state.termsConditionLayout.seo_title || 'SEO Debate resources.'
                },
                {
                    name: 'description',
                    content: this.$store.state.termsConditionLayout.description
                },
                {
                    property: 'og:title',
                    content: this.$store.state.termsConditionLayout.seo_title || 'SEO Debate resources.'
    ...
    computed: {
        allTermsCondition(){
            return this.$store.getters.allTermsCondition
        },
        updatePage: {
            get: function() {
                return this.page;
        ...
        visbleresquestions() {
            return this.allTermsCondition.slice(
    ...
    async asyncData({ $axios, store }) {
        ...
        if(!Object.values(store.state.termsConditionLayout).length){
            let data = await $axios.$get("/page?page=terms-condition");
            store.commit("update_terms_condition_layout", data);
        }
    },
    components: {
        TermsBanner: () =>
            import("@/components/sections/termsConditions/TermsBanner"),
        BannerCategoryCard: () =>
