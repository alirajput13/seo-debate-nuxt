...
export default {
    ...
    computed: {
        relatedresources: function() {
            if(this.selectedResource && this.selectedResource.category){
                return this.$store.getters.allResources.filter(
                    rs =>
    ...
    async asyncData({ $axios, store, params, error }) {
        ...
        let selectedResource = await $axios.$get(`privacyPolicy/${params.slug}`)
        .catch((err) => {
    ...
    mounted(){
        ...
        if(!this.$store.state.allResources.length){
            this.$axios.$get("/privacy-policy").then( res => this.$store.commit("update_resources", res))
        }
