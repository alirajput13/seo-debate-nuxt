      ...
      ----------------------------------------------- -->
        ...
        >
            ...
            <div class="bg-light bottom-spacer no-underline">
                <v-container>
                    <v-row>
                        <v-col v-if="$store.state.resourceLayout.showFAQ" class="bg-light bottom-spacer no-underline" cols="12">
                            <Faqs :faqs="$store.state.resourceLayout.faq" />
                        </v-col>
                    </v-row>
                </v-container>
            </div>
            <div v-if="$store.state.resourceLayout.showB">
                <SeoTips
                    :layout="{
                        h: $store.state.resourceLayout.header2,
                        s: $store.state.resourceLayout.sub2,
                        c: $store.state.resourceLayout.content
                    }"
                />
            </div>
            <!-- <SeoTips /> -->
...
export default {
    ...
    head() {
        return {
            ...
            link: [
                {
                    rel: 'canonical',
                    href: this.$store.state.resourceLayout.canonical_link || 'https://seodebate.com' + this.$route.path
                }
            ],
            meta: [
                {
                    name: 'robots',
                    content: !!+this.$store.state.resourceLayout.index ? 'index,follow' : 'noindex'
                },
                {
                    name: 'title',
                    content: this.$store.state.resourceLayout.seo_title || 'SEO Debate resources.'
                },
                {
                    name: 'description',
                    content: this.$store.state.resourceLayout.description
                },
                {
                    property: 'og:title',
                    content: this.$store.state.resourceLayout.seo_title || 'SEO Debate resources.'
    ...
    computed: {
        ...
        visbleresquestions() {
            return this.allResources.slice(
    ...
    async asyncData({ $axios, store }) {
        ...
        if(!Object.values(store.state.resourceLayout).length){
            let data = await $axios.$get("/page?page=resource");
            store.commit("update_resource_layout", data);
        }
        if(!store.state.allResources.length){
            let resources = await $axios.$get("/resources");
            store.commit("update_resources", resources.sort( (a,b) => b.featured - a.featured));
        }
    },
    components: {
        ResoucesBanner: () =>
            import("@/components/sections/resources/ResoucesBanner"),
        BannerCategoryCard: () =>
            import("@/components/sections/shared/BannerCategoryCard"),
        ResoucesOffer: () =>
            import("@/components/sections/resources/ResoucesOffer"),
        ResoucesQuestionList: () =>
            import("@/components/sections/resources/ResoucesQuestionList"),
